...
class ContratoExport
{
    ...
    public function contratoGenerico($id_mov)
    {
        try{
            ...
            ->select("dc.id_detalle_contratacion",DB::raw('CONCAT(cc.apellido_paterno," ",cc.apellido_materno, " ",cc.nombre) as nombre'), "cc.curp", "cc.rfc", "cd.departamento","cp.puesto","dc.sueldo", "dc.sueldo_neto", "dc.fecha_alta","dc.observacion","dc.id_candidato","dc.id_departamento","dc.id_puesto","ce.empresa","ce.rfc as rfcempresa","dc.id_nomina","rm.id_status","dc.id_sucursal", "ns.sucursal","ncn.nomina",DB::raw("CONCAT(gcd.calle,' #',gcd.numero_exterior, ' ,', gcd.cruzamiento_uno, ' ', gcd.cruzamiento_dos) as calleempresa"),DB::raw("CONCAT(gcd_dos.calle,' #',gcd_dos.numero_exterior, ' ,', gcd_dos.cruzamiento_uno, ' ', gcd_dos.cruzamiento_dos) as calle"))
            ...
            ->where("dc.id_detalle_contratacion",$id_mov)
            ->where("dc.activo",1)
            ->first();
            ...
            $phpword->setValue('FECHAINGRESO',date('d-m-Y',strtotime($detalle_contratacion->fecha_alta)));
            $phpword->setValue('FECHAANTIGUEDAD',date('d-m-Y',strtotime($detalle_contratacion->fecha_alta)));
            ...
            $sueldo_letras = NumerosEnLetras::convertir(intval($detalle_contratacion->sueldo), 'pesos', false, 'Centavos');
